...
Key points from his answers:

1. He uses AI chat daily (Gemini) and tab-completion (Copilot) regularly, but have never used Claude Code or Codex.
2. Monthly AI budget: about ¥3,000.
3. He distrusts AI-generated code because it is often wrong, and they fear losing understanding of their own codebase.
4. He sees AI as useful for eliminating busywork — translation, simple refactoring, lookup — but want to keep thinking for himself.
5. He specifically said: "I want to use AI as a sparring partner to deepen my own understanding, not as a replacement for thought."
6. He added: "I only use prompts for things like 'translate this document to English' or 'refactor this in the one obvious way' — tasks that don't require my brain. I never give vague instructions like 'implement this feature.'"
...
So I decided to fork Roo Code, stripping away everything that doesn't belong in a constrained company workflow: all cloud telemetry, marketplace integrations, and upselling features to create a hyper-focused, local-only agent named Faye. What I focus on are:
*   **Local-first only:** Stripped the Roo Cloud integration, webviews, and marketplace UI. No cloud logins.
*   **No forced tool use.** The upstream system prompt and the harness script required the agent to call a tool every single response. I introduced a new `TaskTextOnlyResponse` event so that Faye can pause and talk naturally — ask a clarifying question, explain a concern, say "I'm confused" — without being forced into weird actions. This single change is philosophically important: it means the agent is allowed to think out loud with you instead of being whipped toward a goal. You also get notified when the agent is waiting for your input.
*   **Streamlined prompts:**  Removed verbose rules, markdown formatting boilerplate and shell-specific command chaining logic, as 2026-era models perform better with less prescriptive scaffolding.

Because I hold the Owner role for the Azure subscription of our project, I know exactly how to architect this to keep future security audits clean. I configured Faye to connect to our Azure Microsoft Foundry setup. 

I also intentionally deployed the following models via Microsoft Foundry, all pay-as-you-go:
...
**Why not GPT-5.5?** It's expensive, and [recent reports](https://www.roborhythms.com/gpt-5-4-vs-gpt-5-5/) document higher hallucination rates and output instability compared to GPT-5.4. Coupled with widespread user complaints about [quality degradation](https://github.com/openai/codex/issues/21942) in the weeks following its launch, it seems the model has again been quietly "lobotomized"—a depressing but familiar pattern of post-release cost management across the AI industry this year. For a newcomer already skeptical of AI tools, GPT-5.5's expensive, "confident-but-wrong" failure mode is the absolute worst possible first impression.

**Why Microsoft Foundry?** Issuing yet another API key from yet another vendor reintroduces every problem we're trying to solve: code leakage risk, cost tracking outside project budgets, security audit headaches. By deploying through Foundry, usage rolls into the existing Azure resource billing. Per-user consumption is visible in the Azure portal. I can explicitly set a ¥3,000/month budget for him.


## The Slack Bridge: Solving the Mobile Constraint

We had our local agent now for newcomers. But how do we solve the physical constraint towards automation and mobility?

Developers, even during work time, want to step away from their screens for a short while. For my personal workflow, I want a focused initial brainstorming session with the AI, agree on a plan, execute it, and then go downstairs to feel the breeze and soak up some sun. But if the agent pauses for a permission check while we are away, development stops, so I always feel a lingering anxiety to stay near the computer.

The constraints in an enterprise environment were brutal: The *only* trusted remote surface allowed on our phones is the company Slack. So instead of bypassing security, I leaned into it. I designed a Slack Approval Bridge workflow for Faye:
...
## Concept: Developer-guided automation

I don’t need an agent that codes while I sleep. I need a workflow that lets me step outside without losing control.

The problem with pure harness-driven loops is that when they go wrong, they go wrong expensively and silently. An agent that's been whipped through 30 turns by a `/goal` evaluator may have produced code that technically passes the stated condition while being architecturally incoherent. The reviewer opens the PR, sees 2,000 lines of generated code with no narrative, and has to reverse-engineer what the agent was trying to do. I've been that reviewer. It's a nightmare, and I cannot afford it in a company project where I have to take responsibility for the code quality.
...
Faye's system prompt change — removing the "must use a tool every response" constraint — is the philosophical core of this fork. It says: You are allowed to pause. You are allowed to say "I'm not sure about this." You are allowed to ask me a question before acting. And the Slack bridge makes that pause reachable from anywhere, so the agent doesn't have to choose between asking and wasting my time at the keyboard.
...
My own preferred workflow is:

1. A concentrated brainstorming session with the agent. Align on the plan, the scope, the test criteria.
2. Hand off a long-running task. Don't enable fast mode — save money.
3. Go downstairs, take a walk, get some sun.
4. When the agent finishes or gets stuck, it pings me on Slack. I respond from wherever I am.

That is still automation. It's just automation with a checkpoint — a human who reviews the direction before the agent burns another 50,000 tokens going the wrong way.
